...
**[在线练习](https%3A//regex101.com/r/DOc5Nu/1)**
...
**元字符描述**.句号匹配任意单个字符除了换行符。[ ]字符种类。匹配方括号内的任意字符。[](#fn_ )否定的字符种类。匹配除了方括号里的任意字符*匹配>=0 个重复的在*号之前的字符。+匹配>=1 个重复的+号前的字符。?标记?之前的字符为可选.{n,m}匹配 num 个大括号之间的字符 (n <= num <= m).(xyz)字符集，匹配与 xyz 完全相等的字符串.|或运算符，匹配符号前或后的字符.\转义字符,用于匹配一些保留的字符 `{ } . * + ? ^ $ \ |`^从开始行开始匹配.$从末端开始匹配.
...
正后发断言 记作`(? The fat cat sat on the mat.
```

#### 4.4 ?<!... 负后发断言

负后发断言 记作 `(? The cat sat on cat.
```
...
**标志描述**i忽略大小写。g全局搜索。m多行修饰符：锚点元字符 `^$` 工作范围在每行的起始。
